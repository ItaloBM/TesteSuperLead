...
interface SendEmailDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (details: { nome: string; enviar_para: string[] }) => void;
  isLoading: boolean;
}
...
export const SendEmailDialog: React.FC<SendEmailDialogProps> = ({
  ...
  const handleSubmit = () => {
    ...
    // Transforma a string de e-mails separados por vírgula em um array
    const emailArray = emails.split(',').map(email => email.trim());

    onSubmit({ nome, enviar_para: emailArray });
  };
