...
export const CnpjDetailModal: React.FC<CnpjDetailModalProps> = ({
  isOpen,
  onClose,
  data,
}) => {
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-2 mt-4 border-b pb-4">
          ...
          <InfoRow
            ...
            value={data.situacao_cadastral?.situacao_atual}
          />
          <InfoRow
            label="Data de Abertura"
            value={new Date(data.data_abertura).toLocaleDateString("pt-BR")}
          />
          ...
          <InfoRow
            ...
            value={data.capital_social?.toLocaleString("pt-BR", {
              style: "currency",
              currency: "BRL",
            })}
          />
        ...
        <div className="mt-4 border-b pb-4">
          ...
          <InfoRow
            ...
            value={data.atividade_principal?.descricao}
          />
          {data.atividade_secundaria &&
            data.atividade_secundaria.length > 0 && (
              <div className="mt-2">
                <p className="text-xs text-gray-500 px-3">
                  Atividades Secundárias
                </p>
                <ul className="list-disc list-inside px-3">
                  {data.atividade_secundaria.map((ativ) => (
                    <li key={ativ.codigo} className="text-sm">
                      {ativ.descricao}
                    </li>
                  ))}
                </ul>
        ...
        {data.quadro_societario && data.quadro_societario.length > 0 && (
          <div className="mt-4">
            <h3 className="font-semibold text-gray-700 mb-2 px-3">
